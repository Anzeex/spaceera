...
const isEmbedMode = query.get("embed") === "1";
...
function applyEmbedConfig() {
  ...
  for (const trait of TRAITS) {
    const value = Number(query.get(trait.key));
    if (Number.isFinite(value)) {
      state.traits[trait.key] = clamp(Math.round(value), 0, MAX_TRAIT_VALUE);
    }
  }
...
function drawShip(traits, stats) {
  ...
  if (isEmbedMode) {
    const bounds = getOpaqueBounds(surface.imageData);
    if (bounds) {
      const padding = 8 * surface.detail;
      const sourceX = Math.max(0, bounds.minX - padding);
      const sourceY = Math.max(0, bounds.minY - padding);
      const sourceWidth = Math.min(surface.pixelWidth - sourceX, bounds.maxX - bounds.minX + 1 + padding * 2);
      const sourceHeight = Math.min(surface.pixelHeight - sourceY, bounds.maxY - bounds.minY + 1 + padding * 2);
      ...
      const targetX = (elements.canvas.width - targetWidth) / 2;
      const targetY = (elements.canvas.height - targetHeight) / 2;
      ctx.drawImage(offscreen, sourceX, sourceY, sourceWidth, sourceHeight, targetX, targetY, targetWidth, targetHeight);
      return;
    }
  }

import { createRNG } from '../galaxy/random.js';
...
function mapInGamePlanetTypeToGeneratorType(planet) {
  const safeName = (planet?.name || 'Aurelia').trim() || 'Aurelia';
  const rawType = String(planet?.type || 'Rocky').toLowerCase();
  const rng = createRNG(`${safeName}:ingame-variant`);

  if (rawType.includes('gas')) {
    return 'gas';
  }

  if (rawType.includes('icy')) {
    return rng.random() < 0.24 ? 'lava' : 'icy';
  }

  if (rawType.includes('rock')) {
    const roll = rng.random();
    if (roll < 0.22) return 'desert';
    if (roll < 0.44) return 'oceanic';
    return 'rocky';
  }

  if (rawType.includes('dwarf')) {
    return 'rocky';
  }
...
export function createPlanetGeneratorPreviewDescriptor(planet) {
  const resolvedType = mapInGamePlanetTypeToGeneratorType(planet);
  return createDescriptor(planet?.name || 'Aurelia', {
    type: resolvedType,
    palette: 'auto',
    size: 54,
    atmosphere: 0,
    cloudDistance: 50,
    cloudColor: undefined,
    lightTarget: { x: 0.62, y: 0.48 },
    autoSize: false,
    autoLight: true,
    rings: false,
  });
...
function renderPlanetSprite(ctx, descriptor, rotation, size, options = {}) {
  ...
  const lowDetail = Boolean(options.lowDetail);
  const hasAtmosphere = descriptor.atmosphere > 0.001;
  const terrainOctaves = lowDetail ? 3 : 5;
  const detailOctaves = lowDetail ? 1 : 3;
  const humidityOctaves = lowDetail ? 2 : 4;
  const cloudOctaves = lowDetail ? 2 : 4;
  const colorOctaves = lowDetail ? 2 : 3;
  const accentOctaves = lowDetail ? 1 : 2;
  const shellOctaves = lowDetail ? 2 : 4;
  ...
  const shellRadius = hasAtmosphere ? 1 + descriptor.cloudShellAltitude : 1;
  ...
  for (let y = 0; y < size; y += 1) {
    for (let x = 0; x < size; x += 1) {
      ...
      if (hasAtmosphere && distanceSq <= shellRadiusSq) {
        ...
        const shellNoise = fbm(shellCloudPoint, descriptor.cloudSeed + 1337, shellOctaves, 2.08, 0.58);
      ...
      const needsCloudNoise = descriptor.type === 'gas' || hasAtmosphere;
      const terrainNoise = fbm(terrainPoint, descriptor.terrainSeed, terrainOctaves, 2, 0.54);
      ...
      const humidity = fbm(humidityPoint, descriptor.terrainSeed + 900, humidityOctaves, 2, 0.58);
      const cloudNoise = needsCloudNoise ? fbm(cloudPoint, descriptor.cloudSeed, cloudOctaves, 2.1, 0.57) : 0;
      ...
      const cloudMask = needsCloudNoise
        ? clamp(
...
function getPlanetRenderMetrics(descriptor, spriteSize, canvasSize, scaleOverride = null) {
  const scale = scaleOverride ?? 3.1;
  return {
    scale,
    center: canvasSize / 2,
...
export function renderPlanetGeneratorPreviewFrame({
  targetCtx,
  targetCanvas,
  spriteCtx,
  spriteCanvas,
  ...
  const metrics = getPlanetRenderMetrics(descriptor, spriteCanvas.width, targetCanvas.width, scaleOverride);

  targetCtx.clearRect(0, 0, targetCanvas.width, targetCanvas.height);
  renderSimpleRing(targetCtx, descriptor, metrics, ringPhase, 'back', descriptor.darkMode);

  spriteCtx.clearRect(0, 0, spriteCanvas.width, spriteCanvas.height);
  renderPlanetSprite(spriteCtx, descriptor, rotation, spriteCanvas.width, { lowDetail });
...
export function createPlanetGeneratorPage(container) {

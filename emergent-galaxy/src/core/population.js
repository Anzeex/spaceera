...
function getInfrastructureLevel(planet, key) {
  return Math.max(0, planet.infrastructure?.[key] ?? 0);
}
...
export function calculatePlanetPopulationGrowth(planet) {
...
export function calculateStarPopulationGrowth(star) {
  return (star.planets ?? []).reduce(
    (sum, planet) => sum + calculatePlanetPopulationGrowth(planet),
    0
  );
}

export function estimatePlanetPeriodsToTarget(planet, targetRatio = 1, maxPeriods = 100000) {
  const populationCap = calculatePlanetPopulationCap(planet);
...
export function estimatePlanetPeriodsToFill(planet, maxPeriods = 100000) {
  return estimatePlanetPeriodsToTarget(planet, 1, maxPeriods);
}

export function estimatePlanetDisplayPeriodsToFill(planet, maxPeriods = 100000) {
  const periods = estimatePlanetPeriodsToFill(planet, maxPeriods);
...
export function estimatePlanetDisplayPeriodsToNinety(planet, maxPeriods = 100000) {
  const periods = estimatePlanetPeriodsToTarget(planet, 0.9, maxPeriods);
  if (!Number.isFinite(periods)) {
    return periods;
  }
...
export function estimateStarPeriodsToFill(star, maxPeriods = 100000) {
  return estimateStarPeriodsToTarget(star, 1, maxPeriods);
}

export function estimateStarDisplayPeriodsToFill(star, maxPeriods = 100000) {
  const periods = estimateStarPeriodsToFill(star, maxPeriods);
  if (!Number.isFinite(periods)) {
    return periods;
  }
...
export function estimateStarPeriodsToTarget(star, targetRatio = 1, maxPeriods = 100000) {
  const planetPeriods = (star.planets ?? []).map((planet) =>
    estimatePlanetPeriodsToTarget(planet, targetRatio, maxPeriods)
  );
...
export function estimateStarDisplayPeriodsToNinety(star, maxPeriods = 100000) {
  const periods = estimateStarPeriodsToTarget(star, 0.9, maxPeriods);
  if (!Number.isFinite(periods)) {
    return periods;
  }
...
export function settlePlanetPopulation(planet, completedIntervals = 1) {
...
export function settleStarPopulation(star, completedIntervals = 1) {
  let changed = false;

  for (const planet of star.planets ?? []) {
    changed = settlePlanetPopulation(planet, completedIntervals) || changed;
  }
